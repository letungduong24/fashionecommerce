    ...
    <div className='max-w-7xl mx-auto p-4 sm:p-6'>
        ...
        <div className="w-full rounded-lg border border-gray-600 p-4">
            <div className="flex justify-between mb-1">
                ...
                <div className="w-fit">
                    <p className={`text-sm font-medium px-1 py-0.5 rounded-lg ${orderDetails && orderDetails.isDelivered ? 'bg-emerald-200 text-emerald-900' : 'bg-amber-300 text-amber-900'}`}>{orderDetails && orderDetails.isDelivered ? 'Đã giao' : 'Đang giao'}</p>
                </div>
            </div>
            <div className="flex justify-between mb-4">
                <p className='text-sm'>{orderDetails && new Date(orderDetails.createdAt).toLocaleDateString()}</p>
                <div className="">
                    <p className={`text-gray-700 text-sm ${orderDetails && orderDetails.isDelivered===false ? 'block' : 'hidden'}`}>Dự kiến giao hàng: {calculateEstimatedDelivery(orderDetails && orderDetails.createdAt)}</p>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div className="p-2 border border-gray-500 rounded-lg">
                    ...
                    <p className="text-gray-600 mb-">{orderDetails && orderDetails.firstName} {orderDetails && orderDetails.lastName}</p>
